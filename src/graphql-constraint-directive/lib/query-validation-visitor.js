...
class InputObjectValidationVisitor {
  ...
  onInputValueDefinition (node) {
    ...
    if (!valueTypeDef && valueTypeAst.kind === Kind.NAMED_TYPE) {
      if (!value) return

      let namedTypeDef = iFieldTypeDef.type
      if (namedTypeDef instanceof GraphQLNonNull) { namedTypeDef = namedTypeDef.ofType }

      if (isInputObjectType(namedTypeDef)) {
        validateInputTypeValue(this.context, namedTypeDef, this.argName, this.variableName, value, this.currentField, iFieldNameFull, this.options)
      } else if (isListType(namedTypeDef)) {
        validateArrayTypeValue(this.context, namedTypeDef, iFieldTypeDef, value, this.currentField, this.argName, this.variableName, iFieldNameFull, this.options)
      }
    } else if (isInputObjectType(valueTypeDef)) {
      // nothing to validate
      if (!value) return
